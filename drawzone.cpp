    ...
    if(actualTool==CURSOR){
        if(ev->buttons().testFlag(Qt::LeftButton)){
            if(textItem){
                ...
                QRectF fmRect = fm.boundingRect(textItem->toPlainText());
                textItem->setPos(point.x()-fmRect.width()/2,point.y()-fmRect.height()/2);
            }
            ...
            if(polygonItem){
                polygonItem->setPos(point.x()-polygonItem->boundingRect().width()/2,point.y()-polygonItem->boundingRect().height()/2);
            }

            if(rectItem){
                rectItem->setRect(point.x()-rectItem->rect().width()/2,point.y()-rectItem->rect().height()/2,rectItem->rect().width(),rectItem->rect().height());
            }

            if(elliItem){
                elliItem->setRect(point.x()-elliItem->rect().width()/2,point.y()-elliItem->rect().height()/2,elliItem->rect().width(),elliItem->rect().height());
            }
